<?php $pageTitle = 'Import Orders (CSV) — Finovo OMS/WMS'; require __DIR__ . '/../partials/layout_start.php'; ?>

<div class="max-w-lg mx-auto p-8">
    <a href="/orders" class="text-sm text-ink/60 hover:underline">&larr; Back to Orders</a>
    <h1 class="text-2xl font-bold mt-2 mb-2">Import Orders (CSV)</h1>
    <p class="text-sm text-ink/60 mb-6">
        CSV columns must be in this order: <strong>customer_name, product_name, quantity, price</strong>
    ...
    <?php if ($imported !== null): ?>
        <div class="border border-ink px-4 py-2 rounded mb-4 text-sm">
            <?= (int) $imported ?> order(s) imported successfully.
        </div>
    <?php endif; ?>
    ...
    <form method="POST" action="/orders/import" enctype="multipart/form-data" class="space-y-4">
        <div>
